...
import dynamo/types/attributes.{type AttributeValue, Binary, Number, String}
    ...
    Get | Update | Delete -> {
      case key_value {
        String(_) | Number(_) | Binary(_) -> {
          ...
          let updated_metadata = case
            dict.get(builder.metadata, "CompositeKey")
          {
          ...
          let updated_json_fields =
            dict.insert(builder.json_fields, "Key", json_key)
        ...
        _ -> builder
        // Invalid key type, return unchanged
      }
    }
    _ -> builder
  }
...
pub fn with_return_consumed_capacity(
  builder: DynamoReq,
  val: String,
) -> DynamoReq {
  let updated_json =
